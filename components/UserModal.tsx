...
export const UserModal: React.FC<UserModalProps> = ({ user, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        className="bg-dark-surface rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200 border border-dark-border"
      ...
      >
        ...
        <div className="relative h-28 bg-gradient-to-br from-dark-bg to-dark-surface border-b border-dark-border">
          ...
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 p-2 bg-black/20 hover:bg-neon-500/20 rounded-full text-dark-text hover:text-neon-400 transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-8 pb-8">
          <div className="relative -mt-12 mb-5 flex justify-between items-end">
            <div className="relative">
              <img 
                src={user.avatar_url} 
                alt={user.login} 
                className="w-24 h-24 rounded-2xl border-4 border-dark-surface shadow-xl bg-dark-bg"
              />
            </div>
            <a 
              href={user.html_url}
              ...
              className="mb-1 px-4 py-2 bg-neon-500 text-white text-xs font-bold rounded-lg hover:bg-neon-400 transition-all flex items-center gap-2 shadow-lg shadow-neon-500/20"
            >
              <Github size={14} />
              View Profile
            </a>
          ...
          <div className="grid grid-cols-4 gap-2 mb-8 bg-dark-bg rounded-xl p-4 border border-dark-border">
